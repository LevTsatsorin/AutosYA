...
if (!in_array($nuevo_estado, $estados_validos)) {
    header("Location: ../gestion_reservas.php?error=estado_invalido");
    exit();
}
...
if ($nuevo_estado === $estado_actual) {
    header("Location: ../gestion_reservas.php?error=mismo_estado");
    exit();
}

// Validar transiciones de estado permitidas
if ($nuevo_estado === 'completada' && $estado_actual !== 'confirmada') {
    header("Location: ../gestion_reservas.php?error=solo_confirmadas_completar");
    exit();
}
...
if ($stmt->execute()) {
    $stmt->close();
    $con->close();
    header("Location: ../gestion_reservas.php?estado_actualizado=" . $nuevo_estado);
    exit();
} else {
    $stmt->close();
    $con->close();
